...
//}
//
//float aggregateCost(int row, int col, int ThisZ, path &p, int rows, int cols, Mat &localMax, Mat &localMin, float ***C, float ***A, int sufferZ, int largePenalty, int smallPenalty){
//
...
//
//	/* 在视差之间进行循环，理解其中的意思 */
//	float* p_localMax_rowAddDiff = localMax.ptr<float>(row + p.rowDiff);
//	float* p_localMin_rowAddDiff = localMax.ptr<float>(row + p.rowDiff);
//	int PreheightRange = ceil((p_localMax_rowAddDiff[col + p.colDiff] - p_localMin_rowAddDiff[col + p.colDiff]) / Z_resolution + 1);
...
//* <作者>    <时间>   <版本编号>    <修改原因>\n
//*/
//void aggregateCosts(int rows, int cols, Mat &localMax, Mat &localMin, float  ***C, float  ****A, float  ***S, float sufferZ, int largePenalty, int smallPenalty){
//	int NumOfPaths = 8;
...
//	{
//		float* p_localMax = localMax.ptr<float>(row);
//		float* p_localMin = localMin.ptr<float>(row);
//
//		for (int col = 0; col < cols; ++col)
//		{
//			for (unsigned int path = 0; path < firstScanPaths.size(); ++path)
//			{
//				int heightRange = ceil((p_localMax[col] - p_localMin[col]) / Z_resolution + 1);
//
//				for (int z = 0; z < heightRange; ++z)
//				{
//					S[row][col][z] += aggregateCost(row, col, z, firstScanPaths[path], rows, cols, localMax, localMin, C, A[path], sufferZ, largePenalty, smallPenalty);
//				}
//			}
//		}
...
//				for (int z = 0; z < heightRange; ++z)
//				{
//					S[row][col][z] += aggregateCost(row, col, z, secondScanPaths[path], rows, cols, localMax, localMin, C, A[path], sufferZ, largePenalty, smallPenalty);
//				}
//			}
//		}
//		//打印进度条
